...
\textbf{Initialize:} Variational parameters $\mathbf{m}_{1:K}, \tau_{1:K}, \mathrm{V}_{1:K}, \boldsymbol\mu_{1:K}, \mathrm{Q}_{1:K}, a_{1:K}, b_{1:K}, \xi_{1:N, 1:K}, \alpha_{1:N}$
 ...
 \While{the ELBO has not converged}{
  ...
  \For{$k \in \{1, \ldots, K\}$}{ %% order of the following updates matters
   ...
   % need separate loop for this b/c each alpha_n needs knowledge of the sum over k of lambda(xi_{nk}
   \For{$n \in \{1, \ldots, N\}$}{ 
       \texttt{}
   ...
   Set $\mu_k \leftarrow Q_k^{-1} \eta_k$\; % update mu_k = Q_k^{-1} eta_k
